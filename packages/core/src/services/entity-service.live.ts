...
export const EntityServiceLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createVersion = <E extends Entity>(entity: E, changeType: "create" | "update", changedFields?: ReadonlyArray<string>) =>
      versionRepo
    ...
    return {
      createDoc: (input) => repo.createDoc(input).pipe(Effect.flatMap((entity) => createVersion(entity, "create"))),
      createCodeRef: (input) => repo.createCodeRef(input).pipe(Effect.flatMap((entity) => createVersion(entity, "create"))),
      createStory: (input) => repo.createStory(input).pipe(Effect.flatMap((entity) => createVersion(entity, "create"))),
      createDiagram: (input) => repo.createDiagram(input).pipe(Effect.flatMap((entity) => createVersion(entity, "create"))),
